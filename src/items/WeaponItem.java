...
public class WeaponItem implements Item {
	...
	public void damage(int amount){
		...
		if(durabilityCur <= 0) broken = true;
	}
	...
	public void fix(int amount){
		durabilityCur += amount;
		if(durabilityCur > 0) broken = false;
	}
	...
	public int durability(boolean MaxCur){
		if(MaxCur == true) return durabilityMax;
	 ...
	 * @param gDurability the maximum/current amount of durability an item has
	 * @param gDie the number of dice used to determine the damage this weapon does
	 * @param gDSides the number of sides on the dice used to determine the damage this weapon does
	 * @param gNumber item #
	 * @param gName item name
	 * @param gIcon the icon for the item
	 * @param gDescription description of the item
	 */
	public WeaponItem(int gDurability, int gDie, int gDSides, int gNumber, String gName, File gIcon, String gDescription, ItemModifier gSuffix, ItemModifier gPrefix){
		number = gNumber;
		name = gName;
		icon = gIcon;
